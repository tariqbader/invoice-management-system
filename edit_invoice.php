...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['action'])) {
    ...
    $client_selection = $_POST['client_selection'] ?? 'existing';
    $client_id = $invoice['client_id']; // Default to existing client

    if ($client_selection === 'existing') {
        $client_id = (int)($_POST['existing_client_id'] ?? $invoice['client_id']);
        if ($client_id <= 0) {
        ...
        if (empty($message)) {
            ...
            $stmt->execute([$client_email, $invoice['client_id']]);
            if ($stmt->fetchColumn()) {
    ...
    if (empty($message)) {
        ...
        $tax_rate = isPositiveNumeric($_POST['tax_rate'] ?? '') ? (float)$_POST['tax_rate'] / 100 : DEFAULT_TAX_RATE / 100;
        ...
        if (empty($items)) {
            ...
            // Update invoice
            try {
                $stmt = $pdo->prepare('UPDATE invoices SET client_id = ?, due_date = ?, notes = ?, tax_rate = ?, subtotal = ?, discount_amount = ?, tax_amount = ?, total = ?, discount_type = ?, discount_value = ? WHERE id = ?');
                $stmt->execute([$client_id, $due_date, $notes, $tax_rate, $subtotal, $discount_amount, $tax_amount, $total, $discount_type, $discount_value, $invoice_id]);
                ...
                $message = 'Invoice updated successfully! Invoice #' . $invoice['invoice_number'];
...
// Determine initial client selection state
$initial_client_selection = 'existing';
?>
...
<body>
    <div class="container">
        <h1>✏️ Edit Invoice: <?php echo htmlspecialchars($invoice['invoice_number']); ?></h1>
        ...
        <form method="post" id="invoiceForm">
            ...
            <div class="radio-group">
                <label>
                    <input type="radio" name="client_selection" value="existing" onchange="toggleClientFields()" checked>
                    Use Existing Client
                </label>
                <label>
                    <input type="radio" name="client_selection" value="new" onchange="toggleClientFields()">
                    Create New Client
                </label>
            </div>
            ...
            <div id="existing-client-section">
                <div class="form-group">
                    ...
                    <select name="existing_client_id" id="existing_client_id" onchange="populateClientDetails()">
                        <?php foreach ($existing_clients as $c): ?>
                            <option value="<?php echo $c['id']; ?>" 
                                    <?php echo ($c['id'] == $invoice['client_id']) ? 'selected' : ''; ?>
                                    data-company="<?php echo htmlspecialchars($c['company'] ?? ''); ?>"
                ...
                <div id="client-preview" class="info-box">
                    <div id="client-view-mode">
                        <div id="client-details">
                            <strong>Selected Client:</strong><br>
                            Name: <?php echo htmlspecialchars($invoice_client['name']); ?><br>
                            <?php if ($invoice_client['company']): ?>
                            Company: <?php echo htmlspecialchars($invoice_client['company']); ?><br>
                            <?php endif; ?>
                            Email: <?php echo htmlspecialchars($invoice_client['email']); ?><br>
                            Phone: <?php echo htmlspecialchars($invoice_client['phone'] ?? 'N/A'); ?><br>
                            Address: <?php echo htmlspecialchars($invoice_client['address'] ?? 'N/A'); ?>
                        </div>
                    ...
                    <div id="client-edit-mode" style="display: none;">
                        <div class="grid-2" style="margin-bottom: 15px;">
                            <div class="form-group" style="margin-bottom: 10px;">
                                <label>Name: *</label>
                                <input type="text" id="edit_client_name" value="<?php echo htmlspecialchars($invoice_client['name']); ?>">
                            </div>
                            <div class="form-group" style="margin-bottom: 10px;">
                                <label>Company:</label>
                                <input type="text" id="edit_client_company" value="<?php echo htmlspecialchars($invoice_client['company'] ?? ''); ?>" placeholder="Optional">
                            </div>
                        </div>
                        <div class="grid-2" style="margin-bottom: 15px;">
                            <div class="form-group" style="margin-bottom: 10px;">
                                <label>Email: *</label>
                                <input type="email" id="edit_client_email" value="<?php echo htmlspecialchars($invoice_client['email']); ?>">
                            </div>
                            <div class="form-group" style="margin-bottom: 10px;">
                                <label>Phone:</label>
                                <input type="tel" id="edit_client_phone" value="<?php echo htmlspecialchars($invoice_client['phone'] ?? ''); ?>">
                            </div>
                        </div>
                        <div class="form-group" style="margin-bottom: 15px;">
                            <label>Address:</label>
                            <input type="text" id="edit_client_address" value="<?php echo htmlspecialchars($invoice_client['address'] ?? ''); ?>">
                        </div>
            ...
            <div id="new-client-section" style="display: none;">
                <div class="grid-2">
                    <div class="form-group">
                        ...
                        <input type="text" name="client_name" id="new_client_name">
                    </div>
                    <div class="form-group">
                        <label>Company:</label>
                ...
                <div class="grid-2">
                    <div class="form-group">
                        <label>Email: *</label>
                        <input type="email" name="client_email" id="new_client_email">
                    </div>
                    <div class="form-group">
                        <label>Phone:</label>
            ...
            <div class="grid-2">
                <div class="form-group">
                    ...
                    <input type="date" name="due_date" required value="<?php echo htmlspecialchars($invoice['due_date']); ?>">
                </div>
                <div class="form-group">
                    <label>Tax Rate (%):</label>
                    <input type="number" step="0.01" name="tax_rate" value="<?php echo htmlspecialchars($invoice['tax_rate'] * 100); ?>">
                </div>
            </div>
            ...
            <div class="grid-2">
                <div class="form-group">
                    ...
                    <select name="discount_type">
                        <option value="none" <?php echo ($invoice['discount_type'] ?? 'none') === 'none' ? 'selected' : ''; ?>>None</option>
                        <option value="percentage" <?php echo ($invoice['discount_type'] ?? '') === 'percentage' ? 'selected' : ''; ?>>Percentage (%)</option>
                        <option value="fixed" <?php echo ($invoice['discount_type'] ?? '') === 'fixed' ? 'selected' : ''; ?>>Fixed Amount ($)</option>
                    </select>
                </div>
                <div class="form-group">
                    <label>Discount Value:</label>
                    <input type="number" step="0.01" name="discount_value" value="<?php echo htmlspecialchars($invoice['discount_value'] ?? 0); ?>" placeholder="e.g., 10 or 100.00">
                </div>
            </div>
            
            <div class="form-group">
                <label>Notes:</label>
                <textarea name="notes" rows="3" placeholder="Additional notes or payment terms..."><?php echo htmlspecialchars($invoice['notes']); ?></textarea>
            </div>
            ...
            <h2>Invoice Items</h2>
            <?php $max_items = max(5, count($invoice_items)); ?>
            <input type="hidden" name="item_count" id="item_count" value="<?php echo $max_items; ?>">
            <table class="items-table" id="itemsTable">
                ...
                <tbody id="itemsBody">
                    <?php for ($i = 0; $i < $max_items; $i++):
                        $it = $invoice_items[$i] ?? ['description' => '', 'quantity' => '', 'unit_price' => '', 'repair_details' => ''];
                    ?>
                    <tr>
                        <td><input type="text" name="item_desc_<?php echo $i; ?>" value="<?php echo htmlspecialchars($it['description']); ?>" placeholder="Service description"></td>
                        <td><input type="number" name="item_qty_<?php echo $i; ?>" step="0.01" value="<?php echo htmlspecialchars($it['quantity']); ?>" placeholder="1"></td>
                        <td><input type="number" name="item_price_<?php echo $i; ?>" step="0.01" value="<?php echo htmlspecialchars($it['unit_price']); ?>" placeholder="0.00"></td>
                    ...
                    <tr class="repair-details-row">
                        <td colspan="4">
                            <div class="repair-details-container">
                                ...
                                <textarea name="item_repair_details_<?php echo $i; ?>" class="repair-details-textarea" placeholder="Describe the work performed, parts replaced, issues fixed, etc..."><?php echo htmlspecialchars($it['repair_details'] ?? ''); ?></textarea>
                            </div>
                        </td>
                    </tr>
                    <?php endfor; ?>
            ...
            <div class="invoice-summary" id="invoiceSummary">
                <div class="summary-row">
                    ...
                    <span class="summary-value">NZ$<span id="subtotal"><?php echo number_format($invoice['subtotal'], 2); ?></span></span>
                </div>
                <div class="summary-row">
                    <span class="summary-label">Discount</span>
                    <span class="summary-value">-NZ$<span id="discountAmount"><?php echo number_format($invoice['discount_amount'], 2); ?></span></span>
                </div>
                <div class="summary-row">
                    <span class="summary-label">Tax breakdown</span>
                    <span class="summary-value">GST (<span id="taxRate"><?php echo number_format($invoice['tax_rate'] * 100, 2); ?></span>%)</span>
                </div>
                <div class="summary-row">
                    <span class="summary-label">Tax total</span>
                    <span class="summary-value">NZ$<span id="taxTotal"><?php echo number_format($invoice['tax_amount'], 2); ?></span></span>
                </div>
                <div class="summary-row total-row">
                    <span class="summary-label">Invoice total</span>
                    <span class="summary-value">NZ$<span id="invoiceTotal"><?php echo number_format($invoice['total'], 2); ?></span></span>
                </div>
                <div class="summary-row">
                ...
                <div class="summary-row balance-row">
                    ...
                    <span class="summary-value">NZ$<span id="balanceDue"><?php echo number_format($invoice['total'], 2); ?></span></span>
                </div>
            </div>
            ...
            <div style="margin-top: 40px; display: flex; gap: 10px;">
                ...
                <a href="pdf_invoice.php?id=<?php echo $invoice_id; ?>" target="_blank" class="btn btn-success">View PDF</a>
            </div>
        </form>
    </div>

    <script>
        let itemIndex = <?php echo $max_items; ?>;
        ...
        function enterEditMode() {
            ...
            const company = preview.getAttribute('data-client-company');
            ...
            document.getElementById('edit_client_company').value = company;
        ...
        function saveClientEdit() {
            ...
            fetch('edit_invoice.php?id=<?php echo $invoice_id; ?>', {
                method: 'POST',
                body: formData
            })
        ...
        // Initialize client preview data on page load
